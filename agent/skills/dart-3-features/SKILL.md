---
name: dart-3-features
description: Applies Dart 3 language features in Flutter/Dart code. Use when writing if-else or switch statements, creating new classes, deciding between a data class and a record, or using patterns and destructuring.
---

# Dart 3 Features Skill

This skill defines how to correctly use Dart 3 language features: branches, patterns, pattern types, and records.
All rules are sourced from [dart.dev/language](https://dart.dev/language).
...
| List | `[p1, p2]` | Matches lists by position. Length must match unless rest element used. |
| Rest element | `...`, `...rest` | Matches arbitrary-length tails or collects remaining elements. |
| Map | `{'key': subpattern}` | Matches maps by key. |
...
|---|---|
| Returning multiple values from a single function | The type is reused across multiple files/features |
| Grouping a few values locally with no reuse | You need methods, encapsulation, inheritance, or `copyWith` |
| You need structural equality with no behavior | The type is part of a public API or long-lived data model |
